...
//新增/编辑充电站
Mock.mock("https://www.demo.com/station/edit", 'post', (options: any, title: any) => {
    const payload: any = options.body ? JSON.parse(options.body) : {}
    console.log("新增/编辑充电站接口收到数据：", payload.id, title)

    // 编辑：如果传入了 id 且存在则更新

    if (payload.title === '编辑充电站信息') {
        if (payload.id) {
            const idx = originalChargingStation.findIndex((s: any) => s.id === payload.id)
            if (idx > -1) {
                // 合并更新字段
                originalChargingStation[idx] = { ...originalChargingStation[idx], ...payload }
                // 更新当前 working copy
                chargingStation = JSON.parse(JSON.stringify(originalChargingStation))
                return {
                    code: 200,
                    success: true,
                    data: "编辑成功",
        ...
        if (payload.id) {
            // 如果传入了 id 则检查是否已存在
            const exists = originalChargingStation.some((s: any) => s.id === payload.id)

            let newItem = {}
            if (exists) {
                // 新增：生成新的 id 并插入（使用最大递增序号）
                const maxNum = originalChargingStation.reduce((max: number, it: any) => {
                    const n = parseInt((it.id || '').replace(/\D/g, ''), 10)
                    return isNaN(n) ? max : Math.max(max, n)
                }, 0)
                const newId = `VXZ${String(maxNum + 1).padStart(5, '0')}`
                newItem = { ...payload, id: newId }
            } else {
                newItem = { ...payload, id: payload.id }
            }

            // 将新数据添加到原始数组的开头（或根据需要 push）
            originalChargingStation.unshift(newItem)
            chargingStation = JSON.parse(JSON.stringify(originalChargingStation))
            return {
                code: 200,
                success: true,
...
Mock.mock("https://www.demo.com/station/delete", "post", (options: any) => {
    console.log("删除充电站接口收到数据：", options)
    ...
    // 从原始数据中移除匹配 id 的项（修改 originalChargingStation 以保持持久性）
    for (const id of idArray) {
        const idx = originalChargingStation.findIndex((s: any) => s.id === id)
        if (idx > -1) originalChargingStation.splice(idx, 1)
    }
    // 更新工作副本
    chargingStation = JSON.parse(JSON.stringify(originalChargingStation))
    return {
...
// 添加站点接口
Mock.mock("https://www.demo.com/addStation", "post", (params: any) => {

    const newStation = JSON.parse(params.body);
    console.log("新建站点", newStation);
    // 生成1000以内的随机数
    const randomCount = Math.floor(Math.random() * 1000);
    const item = {
        position: [Number(newStation.location1), Number(newStation.location2)], // 北京（虚拟坐标）
        ...
        children: [
            {
                ...
                label: "东城区",
                children: [
                    { id: 'c01-d01-s01', label: "东城区充电站01" },
                    { id: 'c01-d01-s02', label: "东城区充电站02" },
                    { id: 'c01-d01-s03', label: "东城区充电站03" },
                    { id: 'c01-d01-s04', label: "东城区充电站04" },
                ]
            },
            {
                id: 'c01-d02',
                label: "西城区",
                children: [
                    { id: 'c01-d02-s01', label: "西城区充电站01" },
                    { id: 'c01-d02-s02', label: "西城区充电站02" },
                    { id: 'c01-d02-s03', label: "西城区充电站03" },
                ]
            },
            {
                id: 'c01-d03',
                label: "朝阳区",
                children: [
                    { id: 'c01-d03-s01', label: "朝阳区充电站01" },
                    { id: 'c01-d03-s02', label: "朝阳区充电站02" },
                    { id: 'c01-d03-s03', label: "朝阳区充电站03" },
                ]
            },
            {
                id: 'c01-d04',
                label: "海淀区",
                children: [
                    { id: 'c01-d04-s01', label: "海淀区充电站01" },
                    { id: 'c01-d04-s02', label: "海淀区充电站02" },
                ]
            },
            {
                id: 'c01-d05',
                label: "丰台区",
                children: [
                    { id: 'c01-d05-s01', label: "丰台区充电站01" },
                    { id: 'c01-d05-s02', label: "丰台区充电站02" },
                ]
            }
...
// ====== chunk upload mock support ======
// 存储每个文件 hash 已上传的 chunk 索引集合
const uploadStateMap: Map<string, Set<number>> = new Map()

// check 已上传分片
Mock.mock(/https:\/\/www\.demo\.com\/check\??.*/, 'get', (req: any) => {
    const url = req.url || ''
    const match = url.match(/[?&]hash=([^&]+)/)
    const hash = match ? decodeURIComponent(match[1]) : ''
    const uploadedSet = uploadStateMap.get(hash) || new Set()
    return {
        code: 200,
        message: 'ok',
...
// 接收单个分片上传
Mock.mock('https://www.demo.com/upload', 'post', (req: any) => {
    // Mock.js 在处理 multipart/form-data 时无法直接解析 FormData，
    // 支持多种 body 格式：优先尝试 JSON (axios 会发送 JSON)，其次回退到字符串正则解析
    try {
        let hash: string = ''
        let index: number = NaN

        // 如果 body 是 JSON 字符串或已解析对象，先尝试解析
        if (req.body) {
            try {
                const parsed = typeof req.body === 'string' ? JSON.parse(req.body) : req.body
                if (parsed && typeof parsed === 'object') {
                    if (parsed.hash) hash = String(parsed.hash)
                    if (parsed.index != null) index = Number(parsed.index)
                }
            } catch (e) {
        ...
        // 如果还没解析到，尝试用字符串匹配（兼容一些 FormData / env）
        if (!hash || Number.isNaN(index)) {
            const bodyStr = req.body || ''
            const hMatch = bodyStr.match(/name=["']?hash["']?\s*[,=]\s*([^&\n\r]+)/) || bodyStr.match(/hash=([^&\n\r]+)/)
            const iMatch = bodyStr.match(/name=["']?index["']?\s*[,=]\s*(\d+)/) || bodyStr.match(/index=(\d+)/)
            if (hMatch && !hash) {
                try { hash = decodeURIComponent(hMatch[1].toString().replace(/[^\w-]/g, '')) } catch (e) { hash = String(hMatch[1]) }
            }
            if (iMatch && Number.isNaN(index)) {
                index = parseInt(iMatch[1], 10)
            }
        }

        if (hash && !Number.isNaN(index)) {
            const set = uploadStateMap.get(hash) || new Set()
            set.add(index)
            uploadStateMap.set(hash, set)
            return { code: 200, message: 'chunk received', index }
        }
    } catch (e) {
        // fallthrough
    }
    // 默认返回成功以便客户端继续
    return { code: 200, message: 'chunk received (mock)' }
})

// 合并分片
Mock.mock('https://www.demo.com/merge', 'post', (req: any) => {
    const payload = req.body ? JSON.parse(req.body) : {}
    const { hash, filename, totalChunks } = payload
    // 简单校验
    const uploadedSet = uploadStateMap.get(hash) || new Set()
    const uploadedCount = uploadedSet.size
    if (uploadedCount >= (totalChunks || 0)) {
        // 模拟合并成功，并清理状态
        uploadStateMap.delete(hash)
        return { code: 200, message: 'merge success', data: { filename } }
    }
    return { code: 400, message: 'missing chunks', uploaded: Array.from(uploadedSet) }
})